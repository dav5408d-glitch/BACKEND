...
const providersAvailability_service_1 = require("./providersAvailability.service");
const ollama_service_1 = require("./ollama.service");
// ============================================
// COST TRACKING (In-memory, should be in DB in production)
// ============================================
const userMonthlyCosts = {};
function getUserMonthlyBudget(plan) {
    switch (plan) {
...
function getPremiumRequestLimit(plan) {
    switch (plan) {
...
function getTotalRequestLimit(plan) {
    switch (plan) {
...
function trackCost(userId, cost) {
...
// ============================================
function analyzeIntentAdvanced(message) {
    const text = message.toLowerCase();
    let domain = 'general';
    let complexity = 'medium';
    let requiresWebSearch = false;
    let suggestedProviderTier = 'mid';
...
// PROMPT OPTIMIZATION
// ============================================
function optimizePrompt(message, intent, plan, isInCheapMode) {
...
// ============================================
function pickOptimalProvider(intent, allowedProviders, plan, isInCheapMode) {
    // Filter to only ENABLED providers
    const enabledProviders = allowedProviders.filter(p => {
        const availability = (0, providersAvailability_service_1.getAvailableProviders)().find(ap => ap.key === p.key);
    ...
    const demoProvider = providers_config_1.PROVIDERS.find(p => p.key === 'demo');
    if (demoProvider) {
...
exports.routerService = {
    async handleMessage({ message, user, searchWeb, requestCount }) {
        ...
        const allProvidersForTier = providers_config_1.PROVIDERS.filter(p => allowedTiers.includes(p.tier));
        // Filter to only enabled providers
        ...
        if (!allowedProviders.length) {
            const available = (0, providersAvailability_service_1.getEnabledProviders)();
            ...
            const fallbackProviders = available.map(ap => providers_config_1.PROVIDERS.find(p => p.key === ap.key)).filter((p) => !!p);
        ...
        if (allowWebSearch && intent.requiresWebSearch) {
            try {
                ...
                const results = await websearch_service_1.webSearchService.search(cleanedMessage);
                searchResults = results;
                const formattedResults = websearch_service_1.webSearchService.formatResultsForContext(results.results);
    ...
    async handleMessageLocal({ message, user, image, imageUrl, searchWeb, requestCount, conversationHistory }) {
        ...
        try {
            let response = await (0, ollama_service_1.ollamaGenerate)(optimizedMessage, model);
        ...
        if (image) {
            ...
            const response = await (0, ollama_service_1.ollamaGenerateWithImage)(optimizedMessage, image, visionModel);
